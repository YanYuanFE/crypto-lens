...
| **日期** | 2026-07-09 |
| **修订** | 2026-07-15（R62：Sparkle 2 自动更新） |
...
9. **应用更新**：集成固定版本的 Sparkle 2；每天检查一次，也支持 About 手动检查；DMG enclosure 必须通过 EdDSA 校验后才可安装。
...
| 额外第三方 Swift Package | v1 仅允许固定版本的 Sparkle 2；行情与业务逻辑不增加第三方依赖 |
...
| 应用更新 | Sparkle 2.9.4；feed 位于仓库根目录 `appcast.xml`；EdDSA 私钥仅保存在维护者 Keychain；默认每日检查、安装前询问 | 获得 Developer ID 后保留 Sparkle，改用正式签名与公证产物 |
...
| T53 | Sparkle update：About 显示 version/build 与手动检查；默认每日检查；feed URL、公钥、EdDSA enclosure、版本号与下载 URL 正确 | PR-10/11/12 |
...
| T66 | Dependency scope：Sparkle 是唯一第三方 package，精确锁定 2.9.4 且提交 `Package.resolved`；tests 仅 XCTest/URLProtocol | PR-1/2b/11/12 |
...
19. **v1 仅引入固定版本 Sparkle 2** — 自动更新复用成熟框架；工程门禁拒绝额外 package，并验证锁定版本、feed 与公钥。
20. **Sparkle EdDSA 与 Apple 信任链分离** — EdDSA 防止 feed 中的更新包被替换；Ad-hoc Beta 仍未获得 Developer ID 身份或公证，不能绕过 Gatekeeper。
...
- **Deps**: 无  
- **Description**: macOS deployment target 14.0；scene graph 仅 MenuBarExtra、无 Settings scene；可运行固定 360×480 non-resizable 空面板；icon-only Label；LSUIElement；zh-Hans String Catalog；Developer ID/Hardened Runtime build settings；bootstrap 阶段无第三方依赖与网络（Sparkle 后续由 PR-12 引入）。
...
- **Deps**: PR-10  
- **Description**: Release Owner 证据签收（日期、CMC Terms/Pricing/Keyless URLs、Keyless/keyed shipping/display 结论、准确 attribution）为前置 hard gate；随后 Developer ID archive、Hardened Runtime、notarize/staple、ZIP/DMG smoke test；v1 App Sandbox disabled；无 CI 重复（已在 PR-2b）。

### PR-12: Sparkle 2 application updates

- **Title**: `feat: add Sparkle application updates`
- **Files**: `App/SparkleUpdateController.swift`；`Info.plist`；`appcast.xml`；Beta signing/build scripts
- **Deps**: PR-10, PR-11
- **Description**: 固定 Sparkle 2.9.4；About 手动检查与每日自动检查；菜单栏应用 gentle reminder delegate；EdDSA feed/enclosure；Ad-hoc Beta 嵌套签名与最小 library-validation 例外；DMG 先发布、appcast 后发布；无行情后台轮询。
...
flowchart LR
  ...
  PR10 --> PR12[PR-12 Sparkle]
  PR11 --> PR12
```

---

*End of design document — Implemented / Release Evidence Pending R62 2026-07-15*
