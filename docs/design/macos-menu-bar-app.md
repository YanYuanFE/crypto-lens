...
| **日期** | 2026-07-09 |
| **修订** | 2026-07-11（R55：实现对齐；Keyless 默认、Demo Key 可选） |
...
应用采用 **Swift + SwiftUI** 分层架构，以 **CoinGecko Keyless Public API 为默认访问方式，可选 Demo API Key 增强**（统一覆盖主流 crypto 与可被索引的 on-chain stock tokens），本地以 **Codable JSON 持久化 Watchlist**，无账号体系。应用以 **Accessory / LSUIElement** 方式运行，不占 Dock，聚焦「一眼价格」场景。
...
8. **免 Key 即用**：未配置凭据时直接使用 Keyless Public API；用户可选输入 Demo Key，candidate 必须先验证成功才写入 Keychain。
...
| 决策点 | v1 实现默认 | 后续可改 |
|--------|-------------|----------|
| 数据源档位 | **CoinGecko Keyless 默认；可选 Demo Key**；同一 API base，Demo 才发送认证 header | Pro/Paid 支持需后续独立设计 |
...
| API Key 来源 | **可选**：用户在 Settings 输入 candidate → 服务端验证成功 → Keychain；Debug 可用 env `COINGECKO_DEMO_API_KEY` 注入 | 禁止把 key 打进 release 二进制 |
...
Menu Bar Icon (icon-only, SF Symbol template)
 └─ Popover Panel (fixed 360 × 480pt, non-resizable)
     ...
     │    └─ Settings Mode
     │         ├─ Search Field hidden
     │         └─ Main Scroll: 可选 Demo Key · 刷新说明 · About（version/build）
...
| 首次可用性 | bootstrap 后直接进入 Watchlist；无 Key 也是完整可用状态，Settings 只提供可选 Demo Key |
...
flowchart TD
  ...
  C0 -->|否| F[稳定可见 200ms 后以 Keyless 或 Demo refresh once]
  F --> G[面板保持打开期间不自动轮询]
  C --> H[用户搜索 / 默认 Keyless]
...
- **Search retry**：offline / timeout / 5xx 的 action 为「重试」，立即对当前 normalized query 发新 generation，不再等待 350ms debounce，仍经过全局 RateLimiter；search 不自动 retry。429 时 Retry disabled 到 `nextAllowedRequestAt`，由 banner 显示剩余时间；已配置 Demo Key 返回 401 时标记该 key 无效并进入 Settings，后续用户动作自动改用 Keyless。
...
- **Keyless 默认态**：不配置 Demo Key 不属于异常或 onboarding 状态；Watchlist、搜索、打开时刷新与手动刷新保持可用，不创建缺 Key banner。
- **Configured Key 失效**：使用已保存 Demo Key 的请求若返回 401，本次操作失败且不自动重试；client 在本进程禁用该 key，显示无效状态，下一次用户动作使用 Keyless。重新验证保存或移除 key 会重置该内存状态。
...
| 网络 | **URLSession + async/await** | Keyless 无认证 header；Demo key 按需注入 Header |
...
Keyless 官方说明为共享 IP 池、约 **10–30 calls/min** 且会动态变化；Demo 额度与频率以官网当前计划为准。两种路径都不支持轮询心智。
...
4. 文档与 About 标明：Keyless/Demo 均非无限实时行情。
...
| 项 | MVP 值 |
|----|--------|
| 档位 | **Keyless 默认；可选 Demo Key** |
| Base URL | `https://api.coingecko.com/api/v3` |
| Auth | Keyless 不发送认证 header；Demo 使用 **`x-cg-demo-api-key: <key>`** |
...
| 无 key UI | 正常可用状态；Settings 显示 Keyless 状态与可选 Demo Key 输入框 |
...
| 错误 | 可重试？ | 策略 | UI |
|------|----------|------|-----|
| `missingAPIKey` | 否 | 只用于拒绝空白 candidate；普通请求不会产生 | Demo Key 输入错误 |
...
- Keyless：默认全局 min interval **3s**；Demo：默认 **750ms**。间隔按实际请求凭据动态选择，并共享同一闸门。
...
|------|------|
| 账号 | 无 |
| API Key | **可选**。candidate 仅存在于 Settings 内存状态；验证成功后才进入 **Keychain**；Configured Key 不回填/复制，只按需显示末四位；禁止 UserDefaults/repo/log；**禁止** 将共享 Demo key 嵌入 **Release** 二进制。Debug 可用 env |
| ToS / 归属 | **Hard release gate**：Release Owner 记录核查日期、条款 URL、Keyless/Demo shipping/display 许可结论与准确 attribution 文案；缺任一项或结论不明确即阻塞发布 |
...
| T4 | `AppConfiguration` defaults/injection：usd、open debounce 200ms、manual cooldown 60s、Keyless 3s、Demo 750ms、Watchlist cap 50 | PR-2 |
...
| T12 | 无 key → Keyless HTTP；请求无 Demo/Pro header 且 URL 正确 | PR-5/10 |
...
| T17 | Keyless/Demo 动态 RateLimiter 间隔正确；429 gate 跨 search/price 生效 | PR-5 |
...
| T58 | Keyless/Demo：同一 base；只有 Demo 发送 header；Pro 不存在；stored-key 401 本次不重试、下一用户动作 Keyless；429 不切源 | PR-5/10 |
...
| T62 | Release evidence：`docs/release.md` 含 Release Owner、日期、Terms/Pricing/Keyless URLs、Keyless/Demo shipping/display 结论、准确 attribution；缺项时 release checklist fail | PR-11 |
...
| P1 | Domain + JSON + **CI** | — |
| P2 | Keyless + optional Demo key + CoinGecko + open-time refresh | mock provider |
...
`AppConfiguration` 注入值：`quoteCurrency=usd`、`openRefreshDebounceMilliseconds=200`、`manualRefreshCooldownSeconds=60`、`keylessMinimumRequestIntervalMilliseconds=3000`、`demoMinimumRequestIntervalMilliseconds=750`、`maxWatchlistCount=50`。v1 仅在 bootstrap 组装，不持久化到 UserDefaults。
...
6. **CoinGecko Keyless 默认、Demo Key 可选；同一 base 且仅 Demo 发送 header；Pro 不支持** — 无账号即可用，同时保留用户自备额度路径。
...
11. **可选 Demo Key 先验证后写 Keychain；Footer 数据归属；Release 不嵌 key** — 无效输入不污染已配置凭证，免 Key 路径不受影响。
...
~~原 OQ「v1 Paid/Pro key」~~ → **已关闭：v1 只支持 Keyless 与可选 Demo，无 tier selector；Pro 后续独立设计。**
...
~~原 OQ「是否 keyless」~~ → **已关闭：v1 默认 Keyless，可选 Demo Key。**
...
- CoinGecko: [Keyless Public API](https://docs.coingecko.com/docs/keyless-public-api)、[Demo/Keyless Endpoint Overview](https://docs.coingecko.com/demo/reference/endpoint-overview)、[API Pricing](https://www.coingecko.com/en/api/pricing)、[Simple Price](https://docs.coingecko.com/reference/simple-price)、[API Key Setup](https://docs.coingecko.com/docs/setting-up-your-api-key)（rate limit、credits、缓存频率以官网为准）
...
- **Deps**: PR-2  
- **Description**: Keyless 默认且不发送认证 header，可选 Demo 使用 `x-cg-demo-api-key`；candidate `/simple/price` 验证成功后原子 Keychain commit；stored-key 401 后下一用户动作降级 Keyless；3s/750ms 动态 min interval、共享 `nextAllowedRequestAt`、可取消 gate/retry、soft-miss。
...
- **Deps**: PR-10  
- **Description**: Release Owner 证据签收（日期、Terms/Pricing/Keyless URLs、Keyless/Demo shipping/display 结论、准确 attribution）为前置 hard gate；随后 Developer ID archive、Hardened Runtime、notarize/staple、ZIP/DMG smoke test；v1 App Sandbox disabled、手动更新且无 feed；无 CI 重复（已在 PR-2b）。
