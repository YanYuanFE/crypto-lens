...
| **日期** | 2026-07-09 |
| **修订** | 2026-07-10（R54：grill-with-docs 完成，进入实现就绪状态） |
| **状态** | **Ready for Implementation** |
| **仓库** | `crypto-lens`（Greenfield：截至评审时仅有本设计文档，无应用代码） |
...
本设计从空仓库起步，采用 **Swift + SwiftUI** 分层架构，以 **CoinGecko Demo API（须 API Key）** 作为 MVP 主数据源（统一覆盖主流 crypto 与可被索引的 on-chain stock tokens），本地以 **Codable JSON 持久化 Watchlist**，无账号体系。应用以 **Accessory / LSUIElement** 方式运行，不占 Dock，聚焦「一眼价格」场景。
...
8. **Demo API Key**：用户输入的 candidate 必须先通过 CoinGecko 验证，成功后才写入 Keychain 成为 **Configured API Key**；无 key 时网络功能降级引导。
...
| Keyless CoinGecko 生产请求 | 仅允许 mock/单元测试；**shipping 必须 Demo key** |
| CoinGecko Pro/Paid tier | v1 不接受 Pro key、不显示 tier selector、不实现 Pro base URL/header 或自动 fallback |
...
| 决策点 | v1 实现默认 | 后续可改 |
|--------|-------------|----------|
| 数据源档位 | **仅 CoinGecko Demo**（必须 Demo API Key） | Paid 支持需后续独立设计 |
...
| API Key 来源 | 用户在 Settings 输入 candidate → 服务端验证成功 → Keychain；Debug 可用 env `COINGECKO_DEMO_API_KEY` 注入 | 禁止把 key 打进 release 二进制 |
| Setup Required | `Configured API Key 缺失 && Watchlist 为空` 时每次打开直接进入 Settings Mode；不持久化 onboarding flag | — |
...
Menu Bar Icon (icon-only, SF Symbol template)
 └─ Popover Panel (fixed 360 × 480pt, non-resizable)
     ...
     │    └─ Settings Mode
     │         ├─ Search Field hidden
     │         └─ Main Scroll: API Key · 刷新说明 · About（version/build）
...
| 首次可用性 | **Setup Required** 时 bootstrap 后直接进入 Settings Mode；Back 仍可查看空 Watchlist；条件未解除前下次打开仍进入 Settings Mode |
...
flowchart TD
  ...
  C --> SR{Setup Required?}
  SR -->|是| S[进入 Settings Mode / 配置 API Key]
  SR -->|否| C0{Watchlist ids 为空?}
  C0 -->|是| E0[显示空状态 / 不发 price 请求]
  C0 -->|否| D{有 Demo API Key?}
  D -->|否| E[横幅引导配置 Key / 仍可浏览缓存与本地编辑]
  D -->|是| F[稳定可见 200ms 后 refreshWatchlistPrices once]
  F --> G[面板保持打开期间不自动轮询]
  C -->|有 Key| H[用户搜索]
  ...
  S --> T[Back 返回空 Watchlist Mode]
  S2 --> T2[Back 返回 Watchlist Mode]
  S --> V[验证 Candidate API Key]
  S2 --> V
...
- **Search retry**：offline / timeout / 5xx 的 action 为「重试」，立即对当前 normalized query 发新 generation，不再等待 350ms debounce，仍经过全局 RateLimiter；search 不自动 retry。429 时 Retry disabled 到 `nextAllowedRequestAt`，由 banner 显示剩余时间；401 action 为「配置 API Key」，进入 Settings Mode 并按既有规则清 query/cancel search。
...
- **移除结果**：Keychain delete 成功后停留 Settings Mode。Watchlist 非空时 Back 回 Watchlist 并显示 missing-key banner；为空时 Setup Required 继续成立。Keychain delete 失败则旧 key 仍为 Configured API Key，显示 Settings 本地错误并允许重试，不清除 cache 或授权状态。
...
- **Setup Required**：由 `Configured API Key 缺失 && Watchlist 为空` 实时派生，不保存 `hasSeenOnboarding`。bootstrap 后若成立，本次打开直接进入 Settings Mode；用户可 Back 查看空 Watchlist，但关闭后若条件仍成立，下次打开仍直接进入 Settings Mode。
- **缺 Key 但有历史数据**：默认进入 Watchlist Mode，显示非阻塞「需要 API Key」横幅；缓存浏览与 Watchlist 增删排序保持可用，在线搜索禁用并提供进入 Settings Mode 的动作。
...
- **全局优先级**：`persistenceFailure / corruptedStore / classificationUnavailable` > `configuredKeyInvalid / missingKey` > `rateLimited` > `offline / timeout / serverError` > `refreshFailed`。高优先级消失后重新选择并显示仍 active 的下一项，不丢弃被遮挡状态。
...
- **Resolve 规则**：`persistenceFailure` 在下一次 Watchlist/cache save 成功后解除；`missingKey / configuredKeyInvalid` 在新 key 验证并 commit 成功后解除；`rateLimited` 在 `nextAllowedRequestAt` 到期后解除；`offline / timeout / serverError / refreshFailed` 在下一次相关网络请求成功后解除。新失败可替换同类 condition 的详情和时间。
...
- **Footer labels**：icon-only commands 必须提供 tooltip 与 accessibility label「设置」「退出 Crypto Lens」。attribution 默认显示紧凑 `Data by CoinGecko` 并打开官方 API 页面；Release gate 若要求准确品牌措辞，则改为当时条款指定的 `Data provided by CoinGecko`，不得省略链接。
...
- **仅使用** CoinGecko search 响应的 **`coins`** 数组；忽略 `nfts` / `exchanges` / categories 等集合。
- **结果排序**：保留 CG 顺序为基底；**symbol 精确匹配**（大小写不敏感）置顶；同 symbol 再按 `marketCapRank` 升序（nil 最后）。
- **结果上限**：展示最多 **20** 条。
...
- **空 Watchlist**：Configured API Key 存在且 Watchlist 为空时，Main Scroll 显示居中 SF Symbol +「暂无关注资产」；不使用 card、不添加说明段落或额外 CTA，固定 Search Field 保持可用。若由最后一项 removal 产生，Removal Batch Undo bar 继续显示并可恢复内容。
...
- **手动 Refresh 状态**：任意 price request in-flight 时显示 progress 并 disabled；manual cooldown 内 disabled，tooltip 显示剩余秒数；Watchlist 为空时 disabled +「暂无关注资产」；缺少/无效 Configured API Key 时 disabled +「请先配置 API Key」。所有状态由 RefreshCoordinator 单一投影。
...
- **Thumb slot**：Watchlist/Search rows 均固定保留 28pt slot；远程 thumb URL 可选且仅允许 HTTPS，缺失或加载失败时静默显示 SF Symbol 占位。**v1 不落盘图片缓存**（内存 `URLSession` 默认缓存即可）；失败不改变 row 布局，不影响 Add/价格。
...
- 价格仍走 **CoinGecko**（`AssetID` = CG coin id）。
...
| 网络 | **URLSession + async/await** | Demo key 经 Header 注入 |
...
2. **冷启动与 bootstrap gate**  
   ...
   - **`isPanelOpen == false` 时禁止任何 CoinGecko 请求**（含预热）。

3. **网络 task 所有权**  
   - `RefreshCoordinator`（`actor` 或与 `PanelViewModel` 绑定的单一结构化 `Task` 树）：  
     - `onOpen`：启动 **200ms 可见性 debounce**；到期后再次确认 panel open、bootstrap ready、API Key 存在且 ids 非空，再执行 `refreshNow(reason: .panelOpened)` 一次。  
...
| 手动刷新 | cooldown = **60s Demo floor** | 避免打同一 CG cache 空转 |
| 失败 | 见 Network retry；UI 保留 last-known | 韧性 |
| 成功 | 内存 + disk cache；记录 `fetchedAt` + 可选 `lastUpdatedAt` | 离线 |

**与 CoinGecko 源新鲜度对齐（重要）**：

- v1 Demo `/simple/price` 数据新鲜度按官方当前约 **~60s** 规划；Keyless/Paid 不属于运行时路径。  
...
#### Demo 月度 credits 预算（硬约束）

Demo 约 **10 000 call credits / 月** ≈ **~333 calls / 天**（以官网为准；按下表产品规划）。

| 场景 | 估算 | 结论 |
|------|------|------|
| 每天打开面板 30 次 | ≈30 price + 偶发 search | 低，适合个人 Demo key |
| 面板打开 **8h/天**，自动 60s 轮询 | 8×60 = **480** price/天 **仅轮询** | **超过 ~333/天**，v1 不采用 |
...
4. 文档与 About 标明：个人 Demo 配额有限，非无限实时行情。

### 离线行为

1. 无网络 / 无 key：缓存价 + `Offline` / `需要 API Key`；搜索不可用于发现新资产，但 **Watchlist 增删排序可用**，当前会话已加载且仍显示的搜索结果可以 Add。  
...
sequenceDiagram
  ...
  participant API as CoinGeckoClient
...
### CoinGecko MVP 认证与端点（冻结）

| 项 | MVP 值 |
|----|--------|
| 档位 | **Demo（必须 API Key）** |
| Base URL | `https://api.coingecko.com/api/v3` |
| Auth | HTTP Header **`x-cg-demo-api-key: <key>`** |
| Keyless | **不用于** 生产请求；测试可用 mock `URLProtocol` |
| Pro/Paid | **v1 不实现**：无 Pro base URL/header、无 tier selector、无自动 fallback |
| Keychain | service `app.cryptolens.coingecko`；account `demo-api-key`；accessible `AfterFirstUnlock`；UI 仅按需显示末四位状态 |
| Debug 注入 | 进程环境变量 `COINGECKO_DEMO_API_KEY` 仅 Debug 配置读取；不自动写入 Keychain，只有用户在 Settings 验证保存才 commit |
| 无 key UI | 非阻塞横幅 + Settings 输入框；Watchlist 本地可用 |
...
| 用途 | Path | Query |
|------|------|-------|
| 搜索 | `GET /search` | `query`（min 2） |
| 批量价 | `GET /simple/price` | `ids`, `vs_currencies`, `include_24hr_change=true`, `include_last_updated_at=true` |
...
enum PriceSource: String, Codable, Sendable {
    case coinGecko
    // future: coinMarketCap, geckoTerminal
}
...
| 错误 | 可重试？ | 策略 | UI |
|------|----------|------|-----|
| `missingAPIKey` | 否 | 不请求 | 配置 Key 横幅 |
| `unauthorized` | 否 | candidate 验证时不保存且不影响旧 key；运行期 Configured API Key 收到 401 时停止网络请求直至 key 变更 | 无效 Key |
...
- Demo：默认全局 min interval **750ms**，且配置不得低于当前官方 100 calls/min 对应的 **600ms** 理论下限；远端 plan 变化时更新配置与测试。  
...
actor CoinGeckoClient: AssetSearching, PriceProviding {
    // loads key from APIKeyStoring per request or cached
    // sets header x-cg-demo-api-key
    // maps HTTP status → NetworkError (401, 429, 1020 body/code if present)
}
```
...
- `quotes` 中的 `PriceQuote` 包含可选 `lastUpdatedAt`，`PriceSource` 为 `"coinGecko"`。  
...
### 2. 多源聚合（CG + CMC + DexScreener / GeckoTerminal）

- 长尾 token 与链上价有利；复杂度高。  
- **决策**：v1 单源 CoinGecko。GeckoTerminal / on-chain 作为 **v2 长尾逃生**，不进 MVP。
...
- **决策**：**拒绝作为 v1 主形态**；curated 仅服务 **kind 分类**，自由文本 CG search 保留。可用 fixture 在 UI mock PR 中模拟搜索。
...
|------|------|
| 账号 | 无 |
| API Key | candidate 仅存在于 Settings 内存状态；验证成功后才进入 **Keychain**；Configured Key 不回填/复制，只按需显示末四位；禁止 UserDefaults/repo/log；**禁止** 将共享 Demo key 嵌入 **Release** 二进制（防共享配额滥用）。Debug 可用 env。**用户自备 key** 为分发默认模型 |
| ToS / 归属 | **Hard release gate**：Release Owner 记录核查日期、条款 URL、Demo shipping/display 许可结论与准确 attribution 文案；缺任一项或结论不明确即阻塞发布，用户自备 Key 不等于自动获得展示许可 |
...
| T4 | `AppConfiguration` defaults/injection：usd、open debounce 200ms、manual cooldown 60s、Demo min interval 750ms、Watchlist cap 50 | PR-2 |
...
| T12 | 无 key → `missingAPIKey`，无 HTTP | PR-5/10 |
...
| T17 | Demo RateLimiter 连续请求间距 ≥750ms；429 gate 跨 search/price 生效 | PR-5 |
...
| T21 | Setup Required：无 Key + 空 Watchlist 每次 open 进入 Settings；Back 可看空态；已有 Watchlist + 无 Key 留在 Watchlist 并禁用搜索 | PR-8/10 |
...
| T39 | Manual Refresh UI：仅 Watchlist 可见；in-flight 显示 progress；cooldown/空列表/缺 Key disabled 且 tooltip 正确；Search/Settings 不出现 | PR-8/10 |
...
| T46 | Empty Watchlist：Configured Key 下显示 icon +「暂无关注资产」且 Search 可用；无额外 card/CTA；删除最后一项后 Undo bar 仍可恢复 | PR-1b/8 |
| T47 | Watchlist full：50 项时新结果 `+` disabled + help，已有 `✓` 可定位；Domain race 返回 full 时保留 Search 状态且仅 local 提示；removal 后恢复 | PR-6/7 |
| T48 | Remove API Key：确认前无副作用；成功取消 tasks/删 Keychain/清 gates 但保留 Watchlist/cache；空/非空派生正确；delete 失败保留旧 key | PR-5/10 |
...
| T58 | Demo-only：Settings 无 tier selector；client 仅 Demo base/header；Pro key/header 与 keyless fallback 不存在；429 不触发切源 | PR-5/10 |
...
| T62 | Release evidence：`docs/release.md` 含 Release Owner、日期、Terms/Pricing URLs、Demo shipping/display 结论、准确 attribution；缺项时 release checklist fail | PR-11 |
...
| P1 | Domain + JSON + **CI** | — |
| P2 | Demo key + CoinGecko + open-time refresh | mock provider |
...
`AppConfiguration` 注入值：`quoteCurrency=usd`、`openRefreshDebounceMilliseconds=200`、`manualRefreshCooldownSeconds=60`、`demoMinimumRequestIntervalMilliseconds=750`、`maxWatchlistCount=50`。v1 仅在 bootstrap 组装，不持久化到 UserDefaults。
...
| 风险 | 严重度 | 缓解 |
|------|--------|------|
| Demo **月 credits** 被频繁刷新耗尽 | **中** | 无自动轮询、open debounce 200ms、手动刷新 60s cooldown、429 gate、预算表教育用户 |
...
6. **CoinGecko Demo + `x-cg-demo-api-key` 为 v1 唯一 tier；Keyless/Pro 不 ship 且不 fallback** — 能力边界与限流行为可预测。  
...
11. **用户自备 API Key 先验证后写 Keychain；Footer 数据归属；Release 不嵌 key** — 无效输入不污染已配置凭证，同时满足安全与 ToS。  
...
~~原 OQ「v1 Paid/Pro key」~~ → **已关闭：v1 Demo-only，无 tier selector/fallback；后续独立设计。**  
...
~~原 OQ「CoinGecko ToS 法律确认」~~ → **已关闭：Release Owner 在 `docs/release.md` 做带日期、URL、结论与 attribution 文案的证据签收；不明确即阻塞 P4。**
~~原 OQ「Launch at Login」~~ → **已关闭：v1 不做；post-v1 候选。**

~~原 OQ「是否 keyless」~~ → **已关闭：v1 必须 Demo key。**
...
- CoinGecko: [API Pricing](https://www.coingecko.com/en/api/pricing)、[Simple Price](https://docs.coingecko.com/reference/simple-price)、[API Key Setup](https://docs.coingecko.com/docs/setting-up-your-api-key)（rate limit、credits、缓存频率以官网为准）
...
### PR-5: API key Keychain + CoinGecko client

- **Title**: `feat(network): Demo API key Keychain, CoinGecko client, RateLimiter, NetworkError`
- **Files**: `APIKeyStore`；`Network/**`；URLProtocol tests T8/T9/T12/T13/T17/T22/T23/T48  
- **Deps**: PR-2  
- **Description**: Demo-only base URL 与 `x-cg-demo-api-key`、无 tier/keyless fallback、candidate `/simple/price` 验证成功后原子 Keychain commit、验证流程不自动 retry、429/401 映射、soft-miss、750ms min interval、共享 `nextAllowedRequestAt`、可取消 gate/retry；无 key 不发请求。
...
- **Title**: `feat(ui): API key settings, quit, CoinGecko attribution, offline banners`
- **Files**: `UI/Settings/*`；mode-aware Header/Footer；Status Banner selector；错误映射  
- **Deps**: PR-5, PR-8  
- **Description**: 独立 Settings Mode、Setup Required 派生入口、Back/`Esc` 回 Watchlist、进入时取消/清空 search、Configured Key 末四位状态、SecureField candidate reveal/remask、field validation errors 与 Keychain commit UI、成功后返回 Watchlist 并按需刷新、确认移除 Configured API Key、单一 Status Banner priority/lifecycle selector、稳定 icon footer + attribution link、2s graceful shutdown、刷新说明、About version/build/产品边界/发行方 primary links、ToS 归属文案、格式化 polish；无 updater；mode 切换不触发 open refresh。
...
- **Deps**: PR-10  
- **Description**: Release Owner 证据签收（日期、Terms/Pricing URLs、Demo shipping/display 结论、准确 attribution）为前置 hard gate；随后 Developer ID archive、Hardened Runtime、notarize/staple、ZIP/DMG smoke test；v1 App Sandbox disabled、手动更新且无 feed；无 CI 重复（已在 PR-2b）。
